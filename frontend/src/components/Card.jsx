...
const PartyCard = ({ title, numberOfVotes, link, imgUrl }) => {
  return (
    <div className="bg-white w-[200px] h-[30vh] shadow-md rounded-md">
      <div className="flex flex-col justify-between items-start p-4">
        <div className="flex gap-2 w-full">
          <img src={imgUrl} className="object-contain w-[80px] h-[60px]" />
          <h2 className=" text-slate-600 text-[15px] font-semibold">{title}</h2>
        </div>
        <div className="flex flex-col mt-2">
          <h3 className="text-sm text-slate-500 font-semibold">Tirada Codka</h3>

          <div className="text-slate-600 text-[17px] font-semibold">
            <CountUp
              start={0}
              end={numberOfVotes}
              duration={2.75}
